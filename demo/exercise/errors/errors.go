...
func ParseTime(input string) (Time,error) {
	...
	if len(components) != 3 {
		return Time{}, &TimeParseError{"Invalidad number of time components", input}
	} else{
		...
		if  minute > 59 || minute < 0 {
			return Time{}, &TimeParseError{"minute out of range: 0 <= hour <=23", fmt.Sprintf("%v", minute)}
		}

		if  second > 59 || second < 0 {
			return Time{}, &TimeParseError{"second out of range: 0 <= hour <=23", fmt.Sprintf("%v", second)}
		}
